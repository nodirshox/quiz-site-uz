...
window.onload = function() {
    try {
        ...
        if (currentPage.length === 0 || currentPage === "index.html") {
            ...
            document.getElementById("correct").innerText = `${correctAnswers} of ${totalQuestions}`;
...
function populateQuestion() {
    ...
    // Find question by id
    const selectedQuestion = subject.questions.filter(e => e.id == questionNumber);
...
function countAnswers(subjectId) {
    ...
    for (let i = 0; i < questions.length; i++) {
        let correctAnswer = questions[i].options.filter(e => e.correct == true)[0];
